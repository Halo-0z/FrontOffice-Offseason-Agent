...
SOURCE_CORRECTION_REMOVED_PLAYER_IDS_E3 = {
    "nba-darius-garland",
    "nba-jalen-green",
}

EXPECTED_PLAYER_COUNT_E3 = 14
EXPECTED_MEMBERSHIP_COUNT_E3 = 14

ALLOWED_BATCH1_TEAMS_E3 = {
    "nba-OKC", "nba-DEN", "nba-BOS", "nba-NYK",
    "nba-CLE", "nba-MIN", "nba-HOU", "nba-IND",
}

ALLOWED_POSITIONS_E3 = {"PG", "SG", "SF", "PF", "C", "G", "F", "FC", "GF"}
...
def test_player_identities_json_exists_with_batch1_players() -> None:
    ...
    assert len(players) == EXPECTED_PLAYER_COUNT_E3, (
        f"Expected {EXPECTED_PLAYER_COUNT_E3} players after source correction, got {len(players)}"
    )
    player_ids = [p.get("player_id") for p in players]
    player_id_set = set(player_ids)
    assert F4B_PLAYER_IDS_E3 <= player_id_set, (
        f"Missing F4-B players: {F4B_PLAYER_IDS_E3 - player_id_set}"
    )
    assert BATCH1_NEW_PLAYER_IDS_E3 <= player_id_set, (
        f"Missing Batch 1 corrected players: {BATCH1_NEW_PLAYER_IDS_E3 - player_id_set}"
    )
    for removed_pid in SOURCE_CORRECTION_REMOVED_PLAYER_IDS_E3:
    ...
    for p in players:
        assert p.get("player_id") in F4B_PLAYER_IDS_E3 | BATCH1_NEW_PLAYER_IDS_E3, (
            f"Unexpected player_id: {p.get('player_id')}"
        )
        ...
        assert p.get("position") in ALLOWED_POSITIONS_E3
...
def test_roster_memberships_json_exists_with_batch1_memberships() -> None:
    ...
    assert len(mems) == EXPECTED_MEMBERSHIP_COUNT_E3, (
        f"Expected {EXPECTED_MEMBERSHIP_COUNT_E3} memberships after source correction, got {len(mems)}"
    )
    ...
    assert F4B_PLAYER_IDS_E3 <= player_ids, (
        f"Missing F4-B players in memberships: {F4B_PLAYER_IDS_E3 - player_ids}"
    )
    for removed_pid in SOURCE_CORRECTION_REMOVED_PLAYER_IDS_E3:
        assert removed_pid not in player_ids, (
            f"Source-corrected player {removed_pid} must NOT appear in any membership"
        )
    assert team_ids <= ALLOWED_BATCH1_TEAMS_E3, (
        f"Unexpected team IDs: {team_ids - ALLOWED_BATCH1_TEAMS_E3}"
    )
