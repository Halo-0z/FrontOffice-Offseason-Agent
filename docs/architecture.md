...
- `proposal_viewer` — Deterministic display layer that formats a `StructuredProposal` + `ProposalEvaluation` into a human-readable text brief or a stable JSON-serializable dict. M5-A implements `format_proposal_brief(proposal, evaluation)`, `build_demo_payload(goal)`, and `build_demo_brief(goal)`. The viewer does NOT call any LLM, does NOT use MCP, does NOT call `transaction_rule_engine` or `trade_simulator`, does NOT re-build or re-evaluate the proposal (it calls `run_goal_and_build_proposal` + `evaluate_structured_proposal` only inside the `build_demo_*` convenience wrappers), does NOT write to disk, and does NOT approve transactions. It only formats existing proposal/evaluation data for display. The companion CLI script `backend/scripts/run_offseason_demo.py` exposes the viewer as a command-line demo with `--team-id` / `--objective` / `--target-position` / `--max-salary` / `--max-candidates` / `--evidence-query` / `--format text|json` flags; it prints to stdout and writes no files.
